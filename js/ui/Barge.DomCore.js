...
(function (Bu, Bs, Ba, Bo)
{
   ...
   Barge.Dom = {
       ...
       * @type {!Barge.Dom.DomHelper |undefined}
       * @private
       */
      ...
      {
         return Bu.matches(el, 'input,[contenteditable]') ||
                Bu.matches(el, 'textarea,[contenteditable]');
      },
      ...
      index        : function (el, selector)
      {
         ...
         {
            if ((el.nodeName.toUpperCase() !== 'TEMPLATE') && (selector === '>*' || Bu.matches(el, selector)))
            {
               index++;
            }
         }
      ...
      alignCentersHorizontally : function (el1, el2)
      {
         ...
         if (Bu.isArray(el2) || Bu.isArrayLike(el2))
         {
            Ba.forEach(el2, function (node)
            {
               ...
               Bu.css(node, { left : (el1.offsetLeft - (elsWidthDifference / 2)) + 'px' });
            })
         }
         else
         {
            ...
            Bu.css(el2, { left : (el1.offsetLeft - (elsWidthDifference / 2)) + 'px' });
         }
       ...
       * @returns {void}
       */
      alignRight               : function (el1, el2) //issue this is anchoring right instead [DONE]
      {
      ...
      {
         Bu.assert(node !== null, 'Barge.Dom.setTextContent expects a non-null value for node');
